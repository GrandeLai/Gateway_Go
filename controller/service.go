...
func (service *ServiceController) ServiceList(c *gin.Context) {
	...
	for _, listItem := range list {
		...
		if serviceDetail.Info.LoadType == public2.LoadTypeHTTP &&
			serviceDetail.HTTPRule.RuleType == public2.HTTPRuleTypePrefixURL &&
		...
		if serviceDetail.Info.LoadType == public2.LoadTypeHTTP &&
			serviceDetail.HTTPRule.RuleType == public2.HTTPRuleTypeDomain {
			serviceAddr = serviceDetail.HTTPRule.Rule
		}
		if serviceDetail.Info.LoadType == public2.LoadTypeTCP {
			serviceAddr = fmt.Sprintf("%s:%d", clusterIP, serviceDetail.TCPRule.Port)
		}
		if serviceDetail.Info.LoadType == public2.LoadTypeGRPC {
			serviceAddr = fmt.Sprintf("%s:%d", clusterIP, serviceDetail.GRPCRule.Port)
		}
		ipList := serviceDetail.LoadBalance.GetIPListByModel()
		counter, err := public2.FlowCounterHandler.GetCounter(public2.FlowServicePrefix + listItem.ServiceName)
		if err != nil {
...
func (service *ServiceController) ServiceStat(c *gin.Context) {
	...
	counter, err := public2.FlowCounterHandler.GetCounter(public2.FlowServicePrefix + serviceDetail.Info.ServiceName)
	if err != nil {
		middleware.ResponseError(c, 2004, err)
		return
	}
...
func (service *ServiceController) ServiceAddHTTP(c *gin.Context) {
	...
	tx = tx.Begin()
	serviceInfo := &dao.ServiceInfo{}
	if exist := serviceInfo.ServiceInfoIsExistByService(tx, serviceInfo); exist {
		tx.Rollback()
	...
	if exist := httpUrl.HttpRuleIsExistByHttpRule(tx, httpUrl); exist {
		tx.Rollback()
...
func (service *ServiceController) ServiceUpdateHTTP(c *gin.Context) {
	...
	if err := info.Update(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 2005, err)
		return
	}
	...
	if err := httpRule.Update(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 2006, err)
		return
	}
	...
	if err := accessControl.Update(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 2007, err)
		return
	}
...
func (admin *ServiceController) ServiceAddTcp(c *gin.Context) {
	...
	if exist := infoSearch.ServiceInfoIsExistByService(lib.GORMDefaultPool, infoSearch); exist {
...
func (admin *ServiceController) ServiceUpdateTcp(c *gin.Context) {
	...
	if err := tcpRule.Update(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 2005, err)
		return
	}
...
func (admin *ServiceController) ServiceAddGrpc(c *gin.Context) {
	...
	if exist := tcpRuleSearch.TcpRuleIsExist(lib.GORMDefaultPool, tcpRuleSearch); exist {
		middleware.ResponseError(c, 2003, errors.New("服务端口被占用，请重新输入"))
		return
	}
	grpcRuleSearch := &dao.GrpcRule{
	...
	if exist := grpcRuleSearch.GrpcRuleIsExist(lib.GORMDefaultPool, grpcRuleSearch); exist {
		middleware.ResponseError(c, 2004, errors.New("服务端口被占用，请重新输入"))
		return
	}
...
func (admin *ServiceController) ServiceUpdateGrpc(c *gin.Context) {
	...
	if err := grpcRule.Update(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 2006, err)
		return
	}
