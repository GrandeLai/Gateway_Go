...
func (admin *APPController) APPList(c *gin.Context) {
	...
	for _, item := range list {
		appCounter, err := public2.FlowCounterHandler.GetCounter(public2.FlowAppPrefix + item.AppID)
		if err != nil {
...
func (admin *APPController) AppAdd(c *gin.Context) {
	...
	if exist := search.AppIsExist(lib.GORMDefaultPool, search); exist {
		middleware.ResponseError(c, 2002, errors.New("租户ID被占用，请重新输入"))
		return
	}
	if params.Secret == "" {
		params.Secret = public2.MD5(params.AppID)
	}
...
func (admin *APPController) AppUpdate(c *gin.Context) {
	...
	info.Qps = params.Qps
	info.Qpd = params.Qpd
	if err := info.Update(c, lib.GORMDefaultPool); err != nil {
		middleware.ResponseError(c, 2003, err)
		return
	}
	middleware.ResponseSuccess(c, "")
...
func (admin *APPController) AppStatistics(c *gin.Context) {
	...
	//今日流量全天小时级访问统计
	todayStat := []int64{}
	counter, err := public2.FlowCounterHandler.GetCounter(public2.FlowAppPrefix + detail.AppID)
	if err != nil {
		middleware.ResponseError(c, 2002, err)
		c.Abort()
		return
