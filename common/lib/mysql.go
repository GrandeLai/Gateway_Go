...
func InitDBPool(path string) error {
	...
	for confName, DbConf := range DbConfMap.List {
		...
		sqlDB, err := sql.Open("mysql", DbConf.DataSourceName)
		...
		dbgorm.SetMaxIdleConns(DbConf.MaxIdleConn)
		dbgorm.SetMaxOpenConns(DbConf.MaxOpenConn)
		dbgorm.SetConnMaxLifetime(time.Duration(DbConf.MaxConnLifeTime) * time.Second)
