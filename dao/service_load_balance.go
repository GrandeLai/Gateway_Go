...
import (
	...
	"Gateway-Go/reverse_proxy/load_balance"
	"fmt"
...
func (t *LoadBalance) Update(c *gin.Context, tx *gorm.DB) error {
...
func (lbr *LoadBalancer) GetLoadBalancer(service *ServiceDetail) (load_balance.LoadBalance, error) {
	...
	ipList := service.LoadBalance.GetIPListByModel()         //需要分隔，数据库的是[100.90.164.31:8072,100.90.163.51:8072,100.90.163.52:8072,100.90.165.32:8072]
	weightList := service.LoadBalance.GetWeightListByModel() //需要分隔，数据库的是[50,50,50,80]
	ipConf := map[string]string{}
	for ipIndex, ipItem := range ipList {
		ipConf[ipItem] = weightList[ipIndex]
	} //创建出{"127.0.0.1:2001":"20","127.0.0.1:2002":"20"}这种map
